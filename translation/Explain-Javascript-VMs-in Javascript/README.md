...
![4](https://github.com/RogerZZZZZ/V8-blog/blob/master/translation/Explain-Javascript-VMs-in%20Javascript/img/4.png)

对于高效的table，比如我们使用的结构化数据需要像C语言中的`structs`一样：一些在固定偏移位置的域的序列。 与table相同的被用作为array：我们想要一些数字属性为存于数组中。但十分明显的是不是所有的table都适用这样的表示方法：一些实际上被使用为table，要不包含非字符串非数字的keys，要不包含很多用字符串命名的属性。不幸的是我们不能执行任何高代价的类型推断，我们只能去发现一种在程序运行时在每个table之后的一种结构，并创建和改变它们。幸运的是有一种有名的技术允许我们这样做，这个技术被称为隐藏类(`hidden classes`)
